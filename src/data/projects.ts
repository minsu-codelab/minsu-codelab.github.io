...
export const projects: Project[] = [
  {
    ...
    paar: {
      ...
      action: {
        ko: 'Claude Code류 에이전트의 두뇌(추론·행동 루프, 도구·권한 제어, 검증 단계)를 직접 구현했습니다. 코드를 실제 실행해 검증 + 여러 관점의 교차 검토 + 답을 반박해 깨뜨려 보는 검토(보이지 않는 논쟁) + 여러 후보 중 최선 선택 + 틀리면 자가 수정 + 확신 없으면 보류. 또 기존에 만든 제 프로젝트 코드를 검색해 참고하도록 학습시키고, 언어·프레임워크 사용법(docs)을 184장 레퍼런스로 사전 학습시켰으며, 검증 통과분만 기억하는 메모리와 7B 모델 직접 파인튜닝(QLoRA)까지 검증했습니다.',
      ...
      result: {
        ko: '생성 정확도 94%(16/17, 3회 반복 동일)·도구 사용 정확도 85~87%를 실측했고, 검증 단계가 자신 없을 때 실제로 답을 보류함을 라이브로 확인했습니다. QLoRA는 RTX 5070(Blackwell)에서 학습이 도는 것을 검증(loss 1.04)했고, pytest 198·mypy strict·CI를 갖춰 GitHub에 공개했습니다.',
    ...
    pipeline: {
      ...
      steps: [
        ...
        { nodes: [{ id: 'g2', label: { ko: '난이도에 따라 모델 선택 (7B↔30B)', en: 'Difficulty routing (7B↔30B)' }, emphasis: true }] },
        { nodes: [{ id: 'g3', label: { ko: '관련 코드 검색 + 검증 기억', en: 'RAG + verified memory' } }] },
        { nodes: [{ id: 'g4', label: { ko: '여러 답안 생성·선택', en: 'Generate · best-of-N' }, emphasis: true }] },
        {
          parallel: true,
          note: { ko: '검증 · 보이지 않는 논쟁', en: 'verify · hidden debate' },
          nodes: [
            { id: 'g5a', label: { ko: '실제 실행 검증', en: 'Execution oracle' }, emphasis: true },
            { id: 'g5b', label: { ko: '여러 관점 교차검토', en: 'Multi-lens judges' }, emphasis: true },
            { id: 'g5c', label: { ko: '반론 검토', en: 'Adversary' }, emphasis: true },
          ],
        },
        { nodes: [{ id: 'g6', label: { ko: '자가수정 → 전달 or 보류', en: 'self-repair → deliver or abstain' }, emphasis: true }] },
      ],
    },
    deploy: {
